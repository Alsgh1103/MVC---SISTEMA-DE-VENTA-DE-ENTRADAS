...
public class ControladorAdminRegistrarConcierto implements ActionListener {
        ...
        if (c != null) {
            ...
            if (vista.lblNombreImagen != null) {
                ...
                if (ruta != null && !ruta.isEmpty()) {
                    ...
                    vista.lblNombreImagen.setText("Imagen actual: " + f.getName());
                } else {
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == vista.btnGuardar) {
            ...
            volver();
        } else if (vista.btnImagen != null && e.getSource() == vista.btnImagen) {
            seleccionarImagen();
        }
    }
    ...
    private void seleccionarImagen() {
        ...
        if (result == JFileChooser.APPROVE_OPTION) {
            ...
            if (vista.lblNombreImagen != null) {
                vista.lblNombreImagen.setText("Imagen seleccionada: " + selectedFile.getName());
            } else {
                JOptionPane.showMessageDialog(vista, "Imagen seleccionada: " + selectedFile.getName(), "Imagen", JOptionPane.INFORMATION_MESSAGE);
            }
        }
    }
    ...
    private void guardarConcierto() {
        try {
            ...
            if (rutaImagenTemporal != null) {
                try {
                    File dir = new File("src/main/resources/img/banners");
                    if (!dir.exists()) {
                        dir.mkdirs();
                    }
                    ...
                    Files.copy(new File(rutaImagenTemporal).toPath(), dest.toPath(), StandardCopyOption.REPLACE_EXISTING);
                    conciertoEditado.setRutaImagen("/img/banners/" + nombreArchivo);
                } catch (IOException ex) {
                    JOptionPane.showMessageDialog(vista, "No se pudo copiar la imagen: " + ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
                }
            }
            ...
            if (contextoCentral.getConciertoSeleccionado() != null && vista.btnGuardar.getText().equals("Guardar Cambios")) {
                JOptionPane.showMessageDialog(vista, "Concierto actualizado exitosamente.", "Éxito", JOptionPane.INFORMATION_MESSAGE);
            } else {
